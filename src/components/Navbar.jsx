...
import {navbarData} from "../assets/JsonsAndDatas/navbarData.jsx";
...
export default function Navbar() {
    ...
    return (
        <nav className={`w-full items-center flex p-[30px] bg-white
                        ${toggle
            ? "fixed z-[1000]"
            : "block"
        }`}>
            <div className="flex justify-between items-center w-full flex-wrap z-1000 md:flex-nowrap">
                ...
                <ul
                    className={`${
                        toggle
                            ? "absolute transition-all duration-500 top-[65px] left-0 w-full z-[10000] bg-white flex flex-col justify-center items-center first:mt-2 "
                            : "top-[-500px] absolute transition-all duration-500 w-full left-0 "
